...
The request-response binding of OSCORE is achieved by the items request_kid / request_piv (and, in group OSCORE, request_kid_context)
that are present in both the request's and the response's AAD, and are hereafter referred to as "request_ details".
...
* The request_ details above can be transported in a Class E option (encrypted and integrity protected) or a Class I option (unencrypted, but part of the AAD hence integrity protected).
...
\[ Suggestion for any OSCORE v2: avoid request_ details in the request's AAD as individual elements. Rather than having 'request_kid', 'request_piv' (and, in Group OSCORE, 'request_kid_context') as separate fields, they can better be something more pluggable.
This would avoid the need to make up an option before processing, and would allow just plugging in the hash or request in there as replacing the elements for the request_ details. \]
...
* It may be present in responses (TBD: Does this affect any other properties?).
  ...
  The Request-Hash option in the response allows populating caches (see below) and enables the decryption of a response sent as a reply to a Deterministic Request.
<!--
MT: I think this should more generally say "to a Consensus Request", since it is supposed to cover not only Deterministic Requests but also and especially Ticket Requests (i.e., Consensus Requests generated by the server itself) to which non-traditional responses are sent as reply.
-->
